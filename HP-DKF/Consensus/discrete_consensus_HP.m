...
x_hat_plus(:,1)=H \ z(:,2);%initial guess
%using least square, z[1]=Hx[1]+v[1],-> x_hat[1]=H\z[1]
%Let x_hat_plus[0]=x_hat[1], because we dont have measurement z[0] at k=0

% χ2-test------------------------------------------------------------------
%Nu(:,1) = z(:,1)-H*x_hat_minus(:,1); % store innovation vectors
%no point to compute 𝜈(1) (𝜈[0]) because x_hat_minus(:,1) is not initialized

% k=2----------------------------------------------------------------------
%detected_k=zeros(1,steps);
for k=2:steps
    %predict
    x_hat_minus(:,k)=Ad*x_hat_plus(:,k-1);
    P_minus{k}=Ad*P_plus{k-1}*Ad'+Q;

    %update

    S{k}   = H*P_minus{k}*H'+R;          % store innovation covariance

    Chi(:,k)    = Nu(:,k)'* S{k}^-1*Nu(:,k);  % χ2-test

    K{k}   = P_minus{k}*H'/S{k};



    x_hat_plus(:,k) = x_hat_minus(:,k) + K{k}*(z(:,k)-H*x_hat_minus(:,k));

    %P_plus{k} = (eye(n)-K{k}*H)*P_minus{k}*(eye(n)-K{k}*H)'+K{k}*R*K{k}';
    P_plus{k}=(eye(n) - K{k}*H)*P_minus{k};
end
